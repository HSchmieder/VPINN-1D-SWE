...
# load probe data
def load_probe_data(n_probe, T_sample, noise_sigma, seed=42):
    ...
    gen = torch.Generator() # fixed noise for reproducibility
    gen.manual_seed(42)
    h_probe = h_probe_clean * torch.normal(mean=1,std=noise_sigma,size=h_probe_clean.size(), generator=gen)
