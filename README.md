...
$\mathcal{L}\_{PDE\_{h}} = \frac{1}{n\_{D} \* n\_{\phi}} \\sum\_{k=1}^{n\_{D}} \\sum\_{l=1}^{n\_{\phi}} \mathcal{L}\_{PDE\_h,k,l}$

*( The momentum conservation loss $\mathcal{L}\_{PDE\_{hu}}$ is computed accordingly)*   
...
The total number of nodes for evaluating PDE losses inside the domain is $n\\_sub^2*n\\_gauss^2$.  
