...
class TrainModel():
    def __init__(self, device, PINN, lr, weights, fct_db, Tscale, Xscale, g, cd, n_sub, n_gauss, n_test, probe_pts, probe_h):
        ...
        self.T = Tscale
        self.X = Xscale
        self.g = ((self.T/self.X)**2*g).to(self.device)
        self.cd = (self.X*cd).to(self.device)
        self.db = (self.X*fct_db(self.X*self.tx_pde[:,1])).reshape(-1,self.n_gauss**2).unsqueeze(1).to(self.device)
        # probe points
    ...
    def weak_pde_residuals(self):
        ...
        u = (self.T/self.X*pred[:,1:]).reshape(-1,self.n_gauss**2).unsqueeze(1)
        # conservation of mass
        unweighted_values_h = (
                         - h*self.dphidt.unsqueeze(0)
                         - h*u*self.dphidx.unsqueeze(0)
                         ).reshape(-1,self.n_gauss**2)
        res_h = unweighted_values_h @ self.gauleg_weights
        # conservation of momentum
        unweighted_values_hu = (
                         - h*u*self.dphidt.unsqueeze(0)
                         - (h*u**2+0.5*self.g*h**2)*self.dphidx.unsqueeze(0)
                         + (self.g*h*self.db + self.cd*u*torch.abs(u))*self.phi.unsqueeze(0)
                         ).reshape(-1,self.n_gauss**2)
